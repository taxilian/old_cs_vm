...
#ifdef TRACEON
#define DOC(FUNC, PARAM1, PARAM2) std::cerr << FUNC << ": " << PARAM1 << ", " << PARAM2 << std::endl;
...
void VirtualMachine::ADD(REGISTER &rd, REGISTER &rs) { DOC("ADD", rd, rs); rd += rs; }
void VirtualMachine::ADI(REGISTER &rd, IMMEDIATE val) { DOC("ADI", rd, val); rd += val; }
void VirtualMachine::SUB(REGISTER &rd, REGISTER &rs) { DOC("SUB", rd, rs); rd -= rs; }
void VirtualMachine::MUL(REGISTER &rd, REGISTER &rs) { DOC("MUL", rd, rs); rd *= rs; }
void VirtualMachine::DIV(REGISTER &rd, REGISTER &rs) { DOC("DIV", rd, rs); rd /= rs; }

void VirtualMachine::LDA(REGISTER &rd, ADDRESS addr) { DOC("LDA", rd, addr); rd = addr; }
void VirtualMachine::LDR(REGISTER &rd, ADDRESS addr) { DOC("LDR", rd, addr); rd = get_int(addr); }
void VirtualMachine::LDR2(REGISTER &rd, REGISTER &rs) { DOC("LDR2", rd, rs); rd = get_int(rs); }
void VirtualMachine::LDB(REGISTER &rd, ADDRESS addr) { DOC("LDB", rd, addr); rd = get_byte(addr); }
void VirtualMachine::LDB2(REGISTER &rd, REGISTER &rs) { DOC("LDB2", rd, rs); rd = get_byte(rs); }
void VirtualMachine::STB(REGISTER &rs, ADDRESS addr) { DOC("STB", rs, addr); set_byte(addr, (char)rs); }
void VirtualMachine::STB2(REGISTER &rs, REGISTER &rd) { DOC("STB2", rs, rs); set_byte(rd, (char)rs); }
void VirtualMachine::STR(REGISTER &rs, ADDRESS addr) { DOC("STR", rs, addr); set_int(addr, rs); }
void VirtualMachine::STR2(REGISTER &rs, REGISTER &rd) { DOC("STR2", rs, rs); set_int(rd, rs); }
void VirtualMachine::MOV(REGISTER &rd, REGISTER &rs) { DOC("MOV", rd, rs); rd = rs; }
...
void VirtualMachine::JMR(REGISTER &r) { DOC("JMR", r, "-"); JMP(r); }
void VirtualMachine::BRZ(REGISTER &r, ADDRESS addr) { DOC("BRZ", r, addr); if (r == 0) JMP(addr); }
void VirtualMachine::BNZ(REGISTER &r, ADDRESS addr) { DOC("BNZ", r, addr); if (r != 0) JMP(addr); }
void VirtualMachine::BLT(REGISTER &r, ADDRESS addr) { DOC("BLT", r, addr); if (r < 0) JMP(addr); }
void VirtualMachine::BGT(REGISTER &r, ADDRESS addr) { DOC("BGT", r, addr); if (r > 0) JMP(addr); }

void VirtualMachine::AND(REGISTER &rd, REGISTER &rs) { DOC("AND", rd, rs); rd = (rs && rd) ? 1 : 0; }
void VirtualMachine::OR(REGISTER &rd, REGISTER &rs) { DOC("OR", rd, rs); rd = (rs || rd) ? 1 : 0; }
void VirtualMachine::CMP(REGISTER &rd, REGISTER &rs) { DOC("CMP", rd, rs); rd = (rd > rs) ? 1 : (rd < rs ? -1 : 0); }
